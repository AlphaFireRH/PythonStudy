...
def ins_img(input_file, img_data, out_file):
    try:
        ...
        for img in img_data:
            ...
            if len(img["str_time"]) > 0:
                if len(img["end_time"]) > 0:
                    ...
                    cmp_str = "overlay=x=%s:y=%s:enable='if(gt(t,%s))'" % (
                        img["x"], img["y"], img["str_time"])
            else:
        ...
        cmd = "ffmpeg -i %s -i %s -filter_complex \"%s\" -y %s" % (
            input_file, img_str_list, complex_png_str, out_file)
...
def video_ins_mute_audio(input_file, mute_mp3_file, out_file):
    try:
        cmd = "ffmpeg -y -i %s -filter_complex '[1:0]apad' -shortest %s" % (
            input_file, mute_mp3_file, out_file)
        res = subprocess.call(cmd, shell=True)
...
def trans_code(input_file, width, height, rate, out_file):
    try:
        cmd = "ffmpeg -y -i %s -s %sx%s -b %sk -acodec copy %s" % (
            input_file, width, height, rate, out_file)
        res = subprocess.call(cmd, shell=True)
...
def playback_speed(input_file, speed, out_file):
    try:
        if speed == "":
            speed = "1"
        cmd = "ffmpeg -y -i %s -filter_complex \"setpts=PTS/%s\" %s" % (
            input_file, speed, out_file)
        res = subprocess.call(cmd, shell=True)
...
def a_v_reverse(input_file, out_file):
    try:
        cmd = "ffmpeg -y -i %s -vf vf reverse -af areverse %s " % (
            input_file, out_file)
        res = subprocess.call(cmd, shell=True)
...
def v_intercept(input_file, str_second, duration, out_file):
    try:
        cmd = "ffmpeg -y -i %s -ss %s -t %s -f mp4 %s" % (
            input_file, str_second, duration, out_file)
        res = subprocess.call(cmd, shell=True)
...
def strict_v_merge(input_file, out_file):
    try:
        cmd = "ffmpeg -y -f concat -safe 0 -i %s -acodec copy %s" % (
            input_file, out_file)
        res = subprocess.call(cmd, shell=True)
...
def video_trans_img(input_file, out_path, img_prefix, category="png"):
    try:
        ...
        out_img = "%s_%s.%s" % (out_path, img, category)
